...
def validate_candidate(candidate: object, label: str, errors: list[str], reference: dict | None = None) -> None:
    ...
    if candidate.get("frequency_type") != "phrase":
        errors.append(f"{label}.frequency_type must be phrase")
    ...
    checked_query = str(candidate.get("checked_query", "")).strip()
    inner = checked_query[1:-1] if len(checked_query) >= 2 and checked_query[0] == checked_query[-1] == '"' else ""
    if not inner or norm(inner) != norm(str(candidate.get("phrase", ""))):
        errors.append(f"{label}.checked_query must quote the same non-empty phrase")
    if reference:
    ...
    if status == "NO_ELIGIBLE_KEYWORD":
        ...
        if not isinstance(rejected, list) or len(rejected) < 3:
            ...
            for index, candidate in enumerate(rejected):
                ...
                if isinstance(frequency, int) and 1000 <= frequency <= 5000:
                    errors.append(f"rejected_candidates[{index}] is in the eligible frequency range")
